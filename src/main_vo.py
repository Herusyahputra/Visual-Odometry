...
if __name__ == "__main__":
    ...
    is_draw_err = True 
    err_plt = Mplot2d(xlabel='Img id', ylabel='Matches',title='Proyeksi Error')

    is_draw_matched_points = True 
    matched_points_plt = Mplot2d(xlabel='Img id', ylabel='Matches',title='Feature matching')
        ...
        if img is not None:
            ...
            if(img_id > 2):	       # start drawing from the third image (when everything is initialized and flows in a normal way)
                ...
                if is_draw_err:         # draw error signals 
                    ...
                    err_plt.draw(errx,'error_x',color='g')
                    err_plt.draw(erry,'error_y',color='b')
                    err_plt.draw(errz,'error_z',color='r')
                    err_plt.refresh()    

                if is_draw_matched_points:
                    matched_kps_signal = [img_id, vo.num_matched_kps]
                    inliers_signal = [img_id, vo.num_inliers]                    
                    matched_points_plt.draw(matched_kps_signal,'1. Matches',color='b')
                    matched_points_plt.draw(inliers_signal,'2. Inliers',color='g')                    
    ...
    if is_draw_traj_img:
        ...
        cv2.imwrite('result/map.png', traj_img)
    if is_draw_3d:
        if not kUsePangolin:
            plt3d.quit()
        else: 
